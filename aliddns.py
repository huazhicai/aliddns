...
timefilehandler = TimedRotatingFileHandler('logs/alidns.log', when='MIDNIGHT', interval=1, backupCount=5)
...
RegionID = os.getenv('REGION_ID', "cn-hangzhou")
...
DomainName = os.getenv('DOMAIN_NAME', 'synvn.top')
HostName = os.getenv('SUB_DOMAIN_NAME', 'seven')
...
def main():
    # 返回阿里云第一页的域名记录
    result = check_records(DomainName)
    ...
    while PageSize * PageNumber < TotalCount:
        # 返回第N页域名记录
        result_2 = check_records(DomainName, PageNumber + 1)
        print(result_2)
